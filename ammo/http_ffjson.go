...
var ffj_key_Http_Body = []byte("Body")
		...
		case fflib.FFParse_want_key:
			...
			if len(kn) <= 0 {
				...
				case 'B':
					...
					if bytes.Equal(ffj_key_Http_Body, kn) {
						currentKey = ffj_t_Http_Body
						state = fflib.FFParse_want_colon
						goto mainparse
					}
				...
				if fflib.SimpleLetterEqualFold(ffj_key_Http_Body, kn) {
					currentKey = ffj_t_Http_Body
					state = fflib.FFParse_want_colon
					goto mainparse
				}
...

	{
		...
		if tok == fflib.FFTok_null {
			...
			for {
				...
				tok = fs.Scan()
				/* handler: tmp_uj__Headers type=string kind=string quoted=false*/

				{
					...
					if tok == fflib.FFTok_null {
						...
						tmp_uj__Headers = string(string(outBuf))

					}
				}

				uj.Headers[k] = tmp_uj__Headers
...
func (mj *HttpJSONDecoder) MarshalJSON() ([]byte, error) {
...
func (mj *HttpJSONDecoder) MarshalJSONBuf(buf fflib.EncodingBuffer) error {
	if mj == nil {
		buf.WriteString("null")
		return nil
	}
...
const (
	ffj_t_HttpJSONDecoderbase = iota
	ffj_t_HttpJSONDecoderno_such_key
)

func (uj *HttpJSONDecoder) UnmarshalJSON(input []byte) error {
	fs := fflib.NewFFLexer(input)
	return uj.UnmarshalJSONFFLexer(fs, fflib.FFParse_map_start)
}

func (uj *HttpJSONDecoder) UnmarshalJSONFFLexer(fs *fflib.FFLexer, state fflib.FFParseState) error {
	var err error = nil
	currentKey := ffj_t_HttpJSONDecoderbase
	_ = currentKey
	tok := fflib.FFTok_init
	wantedTok := fflib.FFTok_init
...
mainparse:
	for {
		...
		case fflib.FFParse_want_key:
			...
			if len(kn) <= 0 {
				// "" case. hrm.
				currentKey = ffj_t_HttpJSONDecoderno_such_key
				state = fflib.FFParse_want_colon
				goto mainparse
			} else {
				switch kn[0] {
		...
		case fflib.FFParse_want_value:
			...
			if tok == fflib.FFTok_left_brace || tok == fflib.FFTok_left_bracket || tok == fflib.FFTok_integer || tok == fflib.FFTok_double || tok == fflib.FFTok_string || tok == fflib.FFTok_bool || tok == fflib.FFTok_null {
				...
				case ffj_t_HttpJSONDecoderno_such_key:
					err = fs.SkipField(tok)
					if err != nil {
						return fs.WrapErr(err)
					}
